...
def f7_divide_dataset_for_oneSVM(df):

    #음성데이터만 존재하는 데이터 프레임
    neg_df= df[df['사망사고여부']==0]
    y_neg= neg_df['사망사고여부']
    X_neg =neg_df.drop(['사망사고여부'],axis=1)
    X_neg_train, X_neg_test, y_neg_train,y_neg_test= train_test_split(X_neg,y_neg,test_size=0.2)

    #양성데이터만 존재하는 데이터프레임
    pos_df= df[df['사망사고여부']==1]
    y_pos= pos_df['사망사고여부']
    X_pos= pos_df.drop(['사망사고여부'],axis=1)

    #테스트셋으로 사용될 데이터 정리(음성 테스트 데이터셋+양성 데이터셋)
    y_test= pd.concat([y_neg_test,y_pos])
    X_test= pd.concat([X_neg_test,X_pos])

    #독립변수와 종속변수를 다시 합친후 로를 섞고 다시 분할하기
    df_test= pd.concat([X_test,y_test],axis=1)
    df_test= df_test.sample(frac=1).reset_index(drop=True)

    X_test= df_test.drop(['사망사고여부'],axis=1)
    y_test= df_test['사망사고여부']

    # #독립변수, 종속변수 나누기 #이거 대신 위의 방법을 사용
    # y=df['사망사고여부']
    ...
    #데이터 언더샘플링(지나치게 많은 y=0 데이터 제거) => one-class SVM에선 사용되지 않음
    # from imblearn.under_sampling import RandomUnderSampler
    # rs = RandomUnderSampler()
    # X_train,y_train= rs.fit_resample(X_train,y_train)

    print(f'X_neg_train의 크기는 {len(X_neg_train)}, y의 크기는 {len(y_neg_train)}')
    print(f'X_test의 크기는 {len(X_test)}, y의 크기는 {len(y_test)}')

    return X_neg_train,X_test,y_test
